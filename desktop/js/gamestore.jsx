...
function gsLoad() {
  try { return JSON.parse(localStorage.getItem(GS_KEY)) || { ov: {}, custom: [] }; }
  catch (e) { return { ov: {}, custom: [] }; }
}
let gsState = gsLoad();
const gsSubs = new Set();

function gsPersist() {
  try { localStorage.setItem(GS_KEY, JSON.stringify(gsState)); } catch (e) {}
  gsSubs.forEach((f) => f());
}
function gsBase() { return (window.CometData && window.CometData.GAMES) || []; }
...
function gameCover(g, ang) {
  if (g && g.cover) return { backgroundImage: `url("${g.cover}")`, backgroundSize: "cover", backgroundPosition: "center" };
  return { background: window.CometData.cover(g.c1, g.c2, ang || 145) };
}

window.GameStore = GameStore;
window.useGames = useGames;
