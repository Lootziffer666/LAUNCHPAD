...
object DailyBudgetReducer {
    fun newDay(previous: TimeBudgetState) = previous.copy(
        grantedBonusTime = 0,
        usedToday = 0,
        unlimitedForToday = false,
        locked = false
    )
}
...
class TimeBudgetController(private val context: Context) {
    ...
    @Synchronized
    fun state(today: LocalDate = LocalDate.now()): TimeBudgetState {
    ...
    fun recordUsage(minutes: Int) {
        ...
        prefs.edit()
            .putInt(
                LaunchpadPrefs.PREF_USED_TODAY,
                state.usedToday + minutes.coerceAtLeast(0)
            )
            .apply()
    }

    @Synchronized
    fun grantBonus(minutes: Int) {
        val state = state()
        prefs.edit()
            .putInt(
                LaunchpadPrefs.PREF_BONUS_TODAY,
                state.grantedBonusTime + minutes.coerceAtLeast(0)
            )
            .apply()
    }

    fun setUnlimitedToday(value: Boolean) =
        prefs.edit().putBoolean(LaunchpadPrefs.PREF_UNLIMITED_TODAY, value).apply()

    fun setLocked(value: Boolean) =
        prefs.edit().putBoolean(LaunchpadPrefs.PREF_REMOTE_LOCKED, value).apply()

    @Synchronized
    fun resetForNewDay(today: LocalDate = LocalDate.now()) {
        val key = today.toString()
        if (prefs.getString(LaunchpadPrefs.PREF_BUDGET_DAY, null) == key) return
        prefs.edit()
            .putString(LaunchpadPrefs.PREF_BUDGET_DAY, key)
            .putInt(LaunchpadPrefs.PREF_USED_TODAY, 0)
            .putInt(LaunchpadPrefs.PREF_BONUS_TODAY, 0)
            .putBoolean(LaunchpadPrefs.PREF_UNLIMITED_TODAY, false)
            .putLong(LaunchpadPrefs.PREF_BREAK_ACTIVE_UNTIL, 0L)
            .putLong(LaunchpadPrefs.PREF_CONTINUOUS_USAGE_START, 0L)
            .apply()
    }
}
