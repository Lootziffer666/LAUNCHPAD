...
object LaunchpadServer {
    ...
    private suspend fun handleStatus(context: Context): Pair<Int, String> {
        ...
        return 200 to JSONObject().apply {
            put("balance", time.remainingToday)
            put("baseDailyBudget", time.baseDailyBudget)
            put("bonusTime", time.grantedBonusTime)
            put("availableToday", time.availableToday)
            put("usedToday", time.usedToday)
            put("remainingToday", time.remainingToday)
            put("unlimitedForToday", time.unlimitedForToday)
            put("locked", time.locked)
            put("breakEnabled", prefs.getBoolean(LaunchpadPrefs.PREF_BREAK_ENABLED, true))
            put("breakAfterMinutes", prefs.getInt(LaunchpadPrefs.PREF_BREAK_AFTER_MINUTES, 60))
            put("breakDurationMinutes", prefs.getInt(LaunchpadPrefs.PREF_BREAK_DURATION_MINUTES, 5))
